...
{
    // Letter constants
    uint256 private constant MAX_TITLE_LEN = 64;
    uint256 private constant MAX_PAGE_LEN = 8192;
    uint256 private constant MAX_AUTHOR_LEN = 64;
    ...
    function initLetter(
        string memory _titleMint,
        string memory _firstPageMint,
        string memory _authorMint,
        address _owner
    ) public initializer {
        ...
        if (getStringLength(_titleMint) != 0) {
            require(
                (getStringLength(_titleMint) <= MAX_TITLE_LEN),
        ...
        if (getStringLength(_firstPageMint) != 0) {
            require(
                (getStringLength(_firstPageMint) <= MAX_PAGE_LEN),
                "Page exceed MAX_PAGE_LEN"
            );
        }
        ...
        if (getStringLength(_authorMint) != 0) {
            require(
                (getStringLength(_authorMint) <= MAX_AUTHOR_LEN),
                "Author exceeds MAX_AUTHOR_LEN"
            );
            _author = _authorMint;
        }
    ...
    modifier onlyReader() {
        if (!_open) {
            require(
                hasRole(READER_ROLE, msg.sender),
                "Restricted to Reader Role"
            );
        }
        _;
    }
    ...
    function isReader(address _account) public view returns (bool) {
        if (_open) {
            return true;
        }
    ...
    function addReader(address _account) public virtual onlyOwner {
    ...
    function removeReader(address _account) public virtual onlyOwner {
    ...
    function readTitle() public view onlyReader returns (string memory) {
        return _title;
    }
    ...
    function readAuthor() public view onlyReader returns (string memory) {
        return _author;
    }
    ...
    function viewPageCount() public view onlyReader returns (uint256) {
    ...
    function getStringLength(string memory _s) private pure returns (uint256) {
    ...
    function writePage(string memory _pageMint) public onlyOwner {
        // Page Body is mandatory, max 65536 characters
        require((getStringLength(_pageMint) != 0), "Cannot mint empty page");
        require(
            (getStringLength(_pageMint) <= MAX_PAGE_LEN),
            "Page exceeds MAX_PAGE_LEN"
        );
